...
order by Reports.OpenDate ASC, Description
...
SELECT ISNULL(CONCAT(FirstName,' ', LastName), 'None') AS FullName,
...
SELECT CASE
			WHEN CONCAT(FirstName,' ', LastName) = NULL THEN 'None'
			WHEN CONCAT(FirstName,' ', LastName) = '' THEN 'None'
...
BEGIN
	...
	DECLARE @diffInHours BIGINT = DATEDIFF(HOUR, @StartDate, @EndDate);
